...
const [baseUrl, label = baseUrl, searchQuery = 'webmcp', repositoryUrl] = process.argv.slice(2);
...
assert.equal(llmsResponse.status, 200, `${label}: /llms.txt must return 200`);
const llms = await llmsResponse.text();
assert.ok(llms.trim().length > 100, `${label}: /llms.txt is unexpectedly empty`);
assert.ok(
  llms.includes(new URL(baseUrl).origin),
  `${label}: /llms.txt does not use the configured public origin`,
);
...
const rejectedOrigin = await fetch(endpoint, {
  ...
  body: JSON.stringify({
    ...
    params: {
      protocolVersion,
      capabilities: {},
      clientInfo: { name: 'candidate-example-smoke', version: '1.0.0' },
    },
  }),
});
